...
from document.models import Document, TesseractClassifier, OcrTesseract
...
def get_threshold_recalculate(q0):
    """Valetov"""
    # threshold for lines detection by y axe
    heights = []
    for i in q0:
        w = i.upper_right_x - i.upper_left_x
        h = i.lower_right_y - i.upper_left_y
        if w == 0: continue
        if w > 0: heights.append(h)
    return np.median(heights)
